...
public:
 ...
 again:     
     ...
     if(ch == 49){//1 сильный удар
	    ...
	    cout<<"Теперь у вас "<<this->ret_heal()<<"Едениц здоровья"<<endl;
		}
 else goto again;
}            
...
//что будет делать компьютер в зависимости от того сколько у него здоровья
 ...
 int heals; //переменная для очков исцеления
	 ...
	 if((this->ret_heal()) >= 67 && (this->ret_heal() <= 100))//При 100 - 67 пр здоровья К
	 {
	 ...
	 if((this->ret_heal() >= 36) && (this->ret_heal() <= 66))//При 66 - 36 пр здоровья К
	 {
	  ...
	  if(temp == 3){ //и с вероятностью 33 пр выберет исцеоение
	  	...
	  	cout<<"У компьютера "<<this->ret_heal()<<" едениц здоровья"<<endl;
	  }
	 } 

	 if((this->ret_heal() >= 1) && (this->ret_heal() <= 35))////При 35 - 1 пр здоровья К
	 {
 ...
 if(first_player == true && i % 2 == 0)           //Если первым ходит игрок
    {
	 ...
	 player.move_player(computer);                //метод хода игрока
    }
 else if(first_player == true && i % 2 == 1){
	 cout<<endl<<tmp<<". Ход компьютера"<<endl<<endl;
	 computer.move_computer(&player); 	 		  //метод хода компьютера
    }
